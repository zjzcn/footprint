...
$.extend($.Valid, {
	...
	prototype: {
		...
		elements: function() {
			...
			var $elements=($.browser && $.browser.msie) ? $(this.currentForm).find(":input") : $([]).add(this.currentForm.elements).filter(":input");
			var _this = this;
		...
		methods: {
			required: function(element, params) {
				...
				case 'select':
					...
					result= options.length > 0 && ( element.type == "select-multiple" || ($.browser && $.browser.msie && !(options[0].attributes['value'].specified) ? options[0].text : options[0].value).length > 0);
					break;
				default:
